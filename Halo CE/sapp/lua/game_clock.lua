...
function OnEventTick()
	local current_mode = get_var(0, "$mode")
	if current_mode ~= "rockets2" and current_mode ~= "sinm" and current_mode ~= "slayerbigass" then
				...
				if player_present(i) then
					...
					if clock_mode[PlayerIndex] ~= nil then
						if clock_mode[PlayerIndex] == 0 then
							...
							rprint(i, "|r"..mm..":"..ss)
							rprint(i, "=========comandos activos==servidor 1v1=====")
							rprint(i, "armas     /rp /ca  /ess /es /sn /ne")
							rprint(i, "vehiculos /gh /rw /ba /wa")
							rprint(i, "solo admins /S3 /vi /dios /invi /pac ")
                            rprint(i, "SI TE DIERON ADMIN INICIA SECION CON /login sns ")
						else
